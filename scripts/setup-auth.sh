...
#
# Writes:
#   /etc/kindred/auth.env      AUTH_SECRET=<32 random bytes base64>   (0600 root:kindred)
#   /etc/kindred/setup-token   <one-time uuid>                        (0600 root:kindred)
#
...
EOF
  ...
  : # leave existing AUTH_SECRET intact
fi
